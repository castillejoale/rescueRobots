...
def rotation_3d(omega, theta):
    ...
    omegaNorm = math.sqrt(omega[0]**2 + omega[1]**2 + omega[2]**2)
    rot = np.identity(3) + omegaHat*math.sin(omegaNorm*theta)/omegaNorm + np.dot(omegaHat,omegaHat)*(1-math.cos(omegaNorm*theta))/(omegaNorm**2)
...
def hat_2d(xi):
    ...
    #YOUR CODE HERE
    xi_hat = np.matrix([[0, -xi[2], xi[0]], [xi[2], 0,xi[1]], [0, 0, 0]])
...
def hat_3d(xi):
    ...
    #YOUR CODE HERE
    xi_hat = np.matrix([[0, -xi[5], xi[4], xi[0]], [xi[5], 0,-xi[3], xi[1]], [-xi[4], xi[3], 0, xi[2]], [0,0,0,0]])
...
def homog_2d(xi, theta):
    ...
    p1 = [[1 - math.cos(xi[2]*theta), math.sin(xi[2]*theta)], [-math.sin(xi[2]*theta), 1-math.cos(xi[2]*theta)]]
    p2 = [[0,-1],[1,0]]
    p3 = [[xi[0]/xi[2]],[xi[1]/xi[2]]]
    p23 = np.dot(p2,p3)
    p = np.dot(p1,p23)
    g1 = np.append(R, p, 1)
    g = np.append(g1, [[0,0,1]], 0)
...
def homog_3d(xi, theta):
    ...
    omegaNorm = math.sqrt(xi[3]**2 + xi[4]**2 + xi[5]**2)
    ...
    g1 = I - R
    g2 = np.dot(omegaHat,v)
    g3 = np.dot(g1,g2)

    print g3

    g4 = np.dot(omega,omega.T)
    g5 = np.dot(v, theta)
    g6 = np.dot(g4,g5)

    print g6

    g7 = (1/omegaNorm**2)*(g3+g6)

    # g7 = (1/omegaNorm**2)*(np.dot(I-R, np.dot(omegaHat,v)) + np.dot(np.dot(omega,omega.T),np.dot(v, theta)))

    g8 = np.append(R, g7, 1)
    g = np.append(g8, [[0,0,0,1]], 0)
    print g
